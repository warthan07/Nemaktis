...
"""LCMaterial class containing the director field data, simulation mesh, and
physics constants"""
class LCMaterial(object):
    ...
    def __init__(self, *, director_field, ne, no, nhost = 1):
        """Construct a Material object for light propagation simulation

        Parameters
        ----------
        director_field : DirectorField object
        ne : float or math string depending on the wavelength variable "lambda" (µm)
            The extraordinary refractive index associated with this material.
        no : float or math string depending on the wavelength variable "lambda" (µm)
            The ordinary refractive index associated with this material.
    ...
    def add_isotropic_layer(self, *, nlayer, thickness):
        ...
        Parameters
        ----------
        nlayer : float or math string depending on the wavelength variable "lambda" (µm)
...
"""DirectorField class storing the director field and mesh informations"""
class DirectorField:
    _mask_type = None
    _mask_formula = None
    _mask_vals = None
    ...
    def __init__(self, **kwargs):
        """Initialize an DirectorField object given either the lengths and
        dimensions of the associated 3D mesh or a path to a vti file containing
        the director field and mesh details.

        In the first version of this constructor, the actual values of the
        director field needs to be provided later using the "init_from_funcs"
        method or via the setter method "vals" (array shape (Nz,Ny,Nx,3)).

        In the second version of this constructor, the values of the director
        field are automatically assigned from the vti file.

        Examples
        --------
        nfield = DirectorField(mesh_lengths=(Lx,Ly,Lz), mesh_dimensions=(Nx,Ny,Nz))
    ...
    def rotate_90deg(self, axis):
        ...
        axis: str
            ...
            the optional parameter 's'='+' or '-' decribes the sign of rotation and 'A'='x', 'y'
            or 'z' defines the rotation axis.
        """
        if axis[-1]=="x":
            ax = 0
        elif axis[-1]=="y":
    ...
    def init_from_funcs(self, nx_func, ny_func, nz_func):
        """Initialize the director field from three vectorizable functions"""
    ...
    def get_pos(ix, iy, iz):
        """Returns the position associated with the mesh indices (ix,iy,iz)
